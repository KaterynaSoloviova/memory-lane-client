    ...
    <header className="bg-gradient-to-r from-[#FCF0D9] to-[#FCF0D9] shadow-sm px-2 sm:px-4 lg:px-6 py-2 sm:py-3 flex flex-col sm:flex-row justify-between items-center border-b border-[#e8ddc7] gap-3 sm:gap-0">
      ...
      <div className="flex items-center gap-12">
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          {/* My Capsules - only show when logged in */}
          {isLoggedIn && (
            <NavLink 
              to="/capsules"
              className="text-[#8B4513] hover:text-[#D2691E] transition-colors font-medium text-base sm:text-lg lg:text-xl"
            >
              My Capsules
            </NavLink>
          )}
          
          {/* About Me - always show */}
          <NavLink 
            to="/about" 
            className="text-[#8B4513] hover:text-[#D2691E] transition-colors font-medium text-base sm:text-lg lg:text-xl"
          >
      ...
      <nav className="flex flex-wrap items-center justify-center sm:justify-end gap-3 sm:gap-4 lg:gap-6 font-medium text-base sm:text-lg lg:text-xl duration-200">
        {isLoggedIn ? (
          <>
            <span className="text-[#8B4513] hover:text-[#A0522D] italic text-xs sm:text-sm font-light">
              Hello, {user?.username}!
            </span>
            <button
              onClick={logOutUser}
